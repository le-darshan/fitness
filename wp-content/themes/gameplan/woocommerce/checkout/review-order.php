...
<div id="order_review">
	...
	<table class="shop_table">
		<thead>
			<!--<tr>
				<th class="product-name"><?php _e( 'PACKAGE / PROGRAM', 'woocommerce' ); ?></th>
                <th class="product-total"><?php _e( 'PRICE', 'woocommerce' ); ?></th>
                <th class="product-total"><?php _e( 'DURATION / MONTHS', 'woocommerce' ); ?></th>
			...
			<?php
				...
				if (sizeof($woocommerce->cart->get_cart())>0) :
					foreach ($woocommerce->cart->get_cart() as $cart_item_key => $values) :
						...
						$duration =  $woocommerce->cart->get_item_data( $values );
									
						$int = filter_var($duration, FILTER_SANITIZE_NUMBER_INT);
						...
						$cat = get_the_terms( $values['product_id'], 'product_cat');
							...
							echo '
								<tr class="' . esc_attr( apply_filters('woocommerce_checkout_table_item_class', 'checkout_table_item', $values, $cart_item_key ) ) . '">
									...
									'<strong><br/><span id="le_packtype">'.$values['variation']['Select Type'].'</span>';
									
									'</td>';
									#echo $woocommerce->cart->get_item_data( $values );
								
									echo'<td class="product-total">' . apply_filters( 'woocommerce_checkout_item_subtotal', $woocommerce->cart->get_product_subtotal( $_product, $values['quantity'] ), $values, $cart_item_key ) . '</td>
									
									<td class="product-duration" style="text-align:center;">' . $program = substr($values['variation']['pa_select-duration'], 0, -7).'</td> 
	...
	<div id="payment">
		...
		<ul class="payment_methods methods" style="visibility:hidden">
			<?php
		...
		<div class="form-row place-order">
			...
			echo apply_filters('woocommerce_order_button_html', '<input type="submit" class="le_paypal" name="woocommerce_checkout_place_order" id="place_order" value="' . $order_button_text . '" />' );
			?>
